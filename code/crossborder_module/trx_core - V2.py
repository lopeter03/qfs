...
class Blockchain:
    ...
    def add_transaction(self, sender, receiver, amount, asset_type="Gold"):
        if not sender or not receiver:
            logging.error("Transaction failed: missing sender or receiver")
            return False
        if amount <= 0:
            logging.error("Transaction failed: amount must be positive")
            return False
        if amount > 1000000:
            logging.error("Transaction failed: amount exceeds limit")
            return False
        if asset_type not in ["Gold", "Commodity", "Currency"]:
            logging.error("Transaction failed: invalid asset type")
            return False
